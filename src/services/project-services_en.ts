...
const projectsServices = () => {
     ...
     const getAllProjects = async () => {
          ...
          try {
               ...
               const data: any = await db.collection('Projects_en').find({}).toArray()
               return data;
          } catch (error: any) {
     ...
     const getAllProjectLinks = async () => {
          ...
          try {
               const db = client.db('LDA_DB')
               let data: Project[] = await db.collection('Projects_en').distinct('projectURL')
               return data
          } catch (error: any) {
               console.log({ message: error.message })
          }
     ...
     const getProjectByLink = async (link: string) => {
          ...
          try {
               const db = client.db('LDA_DB')
               let data: Project[] = await db.collection('Projects_en').find({ link: link }).toArray()
               return data[0]
          } catch (error: any) {
               console.log({ message: error.message })
          }
