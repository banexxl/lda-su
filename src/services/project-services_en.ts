...
export const projectsServices_en = () => {
     ...
     const getAllProjectSummaries = async () => {
          ...
          try {
               const db = client.db('LDA_DB');
               const data: any = await db.collection('ProjectSummaries_en').find({}).sort({ projectEndDateTime: -1 }).toArray()
               return data;
          } catch (error: any) {
               console.log({ message: error.message })
          } finally {
     ...
     const getInProgressProjectSummaries = async () => {
          ...
          try {
               const db = client.db('LDA_DB');
               const data: any = await db.collection('ProjectSummaries_en').find({ 'status': 'in-progress' }).sort({ projectEndDateTime: -1 }).toArray()
               return data;
          } catch (error: any) {
               console.log({ message: error.message })
          } finally {
     ...
     const getCompletedProjectSummaries = async () => {
          ...
          try {
               const db = client.db('LDA_DB');
               const data: any = await db.collection('ProjectSummaries_en').find({ 'status': 'completed' }).sort({ projectEndDateTime: -1 }).toArray()
               return data;
          } catch (error: any) {
               console.log({ message: error.message })
          } finally {
     ...
     const getRandomCompletedProjectSummaries = async () => {
          ...
          try {
               const db = client.db('LDA_DB');
               const data: any = await db.collection('ProjectSummaries_en').aggregate([
     ...
     const getProjectSummaryByLink = async (link: string) => {
          ...
          try {
               const db = client.db('LDA_DB')
               let data: ProjectSummary = await db.collection('ProjectSummaries_en').find({ 'projectSummaryURL': link }).toArray()
               return data
          } catch (error: any) {
               console.log({ message: error.message })
          }
     ...
     const getSearchTermResults = async (searchTerm: string) => {
          ...
          //const collections = ['Activities', 'Projects', 'ProjectSummaries_en']; mora samo projects jer url-ovi u activities i project summaries nemju 'prefix'
