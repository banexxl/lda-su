...
import { LandingNewsItem } from './landing-news-item';
import { LandingNewsItemCarousel } from './landing-news-item-carousel';
...
export const LandingNews = ({ posts }: Props) => {
        ...
        <Box sx={{ overflow: 'hidden', position: 'relative' }}>
          ...
          >
            <Carousel ref={carousel.carouselRef} {...carousel.carouselSettings}>
              {posts.map((post) => (
                <LandingNewsItemCarousel key={post.id} post={post} />
              ))}
            </Carousel>
          </CarouselArrows>
        </Box>
        ...
        >
          ...
          <Stack spacing={3}>
            {posts.slice(0, 3).map((post) => (
              <LandingNewsItem key={post.id} post={post} />
            ))}
          </Stack>
