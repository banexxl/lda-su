...
import { LandingBlogItem } from './landing-blog-item';
import { LandingBlogItemCarousel } from './landing-blog-item-carousel';
...
export const LandingBlog = ({ posts }: Props) => {
        ...
        <Box sx={{ overflow: 'hidden', position: 'relative' }}>
          ...
          >
            <Carousel ref={carousel.carouselRef} {...carousel.carouselSettings}>
              {posts.map((post) => (
                <LandingBlogItemCarousel key={post.id} post={post} />
              ))}
            </Carousel>
          </CarouselArrows>
        </Box>
        ...
        >
          ...
          <Stack spacing={3}>
            {posts.slice(0, 3).map((post) => (
              <LandingBlogItem key={post.id} post={post} />
            ))}
          </Stack>
